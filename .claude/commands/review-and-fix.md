...
3. **Simplify Code**: Use the `code-simplifier:code-simplifier` agent to refine and simplify the recently modified code while preserving functionality.

4. **Address ALL GitHub Comments**: Check the PR on GitHub for any inline review comments and address ALL of them:
   ...
   - Fix EVERY comment's concern regardless of priority (high, medium, low)
...
5. **Verify All Comments Addressed**: Before final push, review the GitHub comments list again:
...
6. **Push Changes**: Commit and push all fixes to the remote branch with a descriptive commit message.
